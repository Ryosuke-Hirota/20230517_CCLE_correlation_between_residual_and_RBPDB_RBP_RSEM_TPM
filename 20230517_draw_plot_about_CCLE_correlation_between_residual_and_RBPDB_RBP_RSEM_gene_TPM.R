...
# This script uses RSEM gene TPM data, not gene RPKM data 
# 2023/05/17 made
...
setwd("C:/Rdata")
dir.create("20230517_CCLE_correlation_between_residual_and_RBPDB_RBP_RSEM_TPM")

# import list about name of pri-miRNA, mature miRNA, and transcript
...
# import data about expression levels of RBPDB RBPs
# this data is located in "//fsw-q02/okamura-lab/Files_related_to_M1_Projects/Hirota/CCLE_Data"
setwd("C:/Rdata/CCLE_data")
RBP.exp <-read.table("CCLE_RNAseq_rsem_410RBPs_tpm.txt",sep="\t",header = T,stringsAsFactors = F,check.names = F)
RBP <-sort(RBP.exp[,2])
...
site.df <-read.table("RNAseq_of_18proteins.txt",sep="\t",header = T,stringsAsFactors = F,check.names = F)
site.df <-site.df[,c(1,20)]
site <-sort(unique(site.df[,2]))
...
# list files of residulas in colon
...
for (i in 1:length(r.file)) {
  for (k in 1:nrow(RBP.exp)) {
    ...
    cell <-res.df[,4]
    res.df <-res.df[,c(-1,-2)]
    res.df[,2] <-gsub("^(X)","",res.df[,2])
    
    # extract rows of cell that exsit residual data from RBP expression data 
    RBP.df <-RBP.exp[RBP.exp[,2]==RBP[k],]
    RBP.df <-as.data.frame(t(RBP.df),stringsAsFactors = F)
    RBP.df[,2] <-rownames(RBP.df)
    rownames(RBP.df) <-NULL
    RBP.df <-RBP.df[c(-1,-2),]
    colnames(RBP.df) <-c(RBP[k],"CELL")
    
    #merge residual data and RBP expression data
    merged.df <-merge(res.df,RBP.df,by="CELL")
    merged.df <-merged.df[,c(2,4,1,3)]
    ...
    if(class(r)!="try-error"){
      ...
      setwd(paste0("C:/Rdata/20230517_CCLE_correlation_between_residual_and_RBPDB_RBP_RSEM_TPM/",f.name))
      #generate plot 
      cp <-ggplot(data=merged.df,aes(x=merged.df[,1],y=merged.df[,2]))+
        ...
        labs(title=paste0("R =",signif(r$estimate,3),", p = ",signif(r$p.value,3),", n = ",nrow(merged.df)),x=paste0("biogenesis efficiency of ",name.df[i,1],"_",name.df[i,2],"_",name.df[i,3]),
             y=paste0("Log2 (TPM+1) of ",RBP[k]))+ #write main title and la title
    ...
    if(i==289&&k==410){
      ...
      setwd("C:/Rdata/20230517_CCLE_correlation_between_residual_and_RBPDB_RBP_RSEM_TPM")
      write.table(sm,"summary_of_correlation_between_residual_and_RBPDB_RBP_RSEM_TPM.txt",sep="\t",row.names = F,quote = F)
    }
  }}
